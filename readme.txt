
Solita Citybike stations and customers drived distances.
Program works in cloud with Azure SQL -server and offers API for frontend program.
Data is configured to take first 50000 trips from May 2021 and all stations for faster loading performance. Deployed with CI/CD pipelines Azure.

Frontend Git: 
https://github.com/gittjar/CitybikeFrontend


API in Cloud >

Swagger:
https://citybikeapi.azurewebsites.net/

All stations:
https://citybikeapi.azurewebsites.net/api/Stations

Stations by ID:
https://citybikeapi.azurewebsites.net/api/Stations/23

Stations by name:
https://citybikeapi.azurewebsites.net/api/StationsByName/haaga

CitybiketripsMay2021 server pagination:
https://citybikeapi.azurewebsites.net/api/CitybikeTripsMay2021?pageNumber=1&pageSize=500

All citybiketrips:
https://citybikeapi.azurewebsites.net/api/CitybikeTrips
